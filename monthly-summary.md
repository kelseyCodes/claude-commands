...
- `/monthly-summary` - Generate summary for current month and upload to GitHub
- `/monthly-summary 2025-10` - Generate summary for October 2025 and upload to GitHub
- `/monthly-summary --username=otheruser` - Use different GitHub username
- `/monthly-summary --local-only` - Save locally only, skip GitHub upload
- `/monthly-summary 2025-09 --username=otheruser --local-only` - Combine options
...
Extract the month/year from the command:
{{RAW_PROMPT}}
...
```bash
gh pr list --repo babylist/web --author=USERNAME --search "created:START_DATE..END_DATE" --state=all --limit=100 --json number,title,state,url,createdAt,mergedAt,additions,deletions,files,labels,body
```

**PRs reviewed by the user (excluding their own):**

```bash
gh pr list --repo babylist/web --search "reviewed-by:USERNAME -author:USERNAME created:START_DATE..END_DATE" --state=all --limit=100 --json number,title,url,state
```
...
- Extract linked tickets (JIRA format like `[TICKET-123]`, `UC-123`, `HL-123`, `GW-123`, `SHOP-123`, etc.)
- Identify project area from file paths in the `files` field:
  - `packs/vendors/` → Vendors & Supply Chain
  - `packs/gift_card/` → Gift Cards
  - `packs/inbound/` → Inbound & Fulfillment
  - `packs/outbound/` → Outbound & Fulfillment
  - `packs/product/` → Product Catalog
  - `packs/registry/` → Registry
  - `packs/health/` → Health/DME
  - `packs/accounting/` → Accounting
  - `db/migrate/` → Database/Infrastructure
  - `spec/` → Testing Infrastructure
  - Other packs → infer from pack name
